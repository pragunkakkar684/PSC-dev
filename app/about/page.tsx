import { ArrowRight, Sparkle, ShieldCheck, Infinity as InfinityIcon } from 'lucide-react';
...
export async function generateMetadata(): Promise<Metadata> {
  return buildPageMetadata('page', 'about', {
    title: 'About Us | PSC Global',
    description:
      'Building trust through expertise, integrity, and long-term partnerships across global regulatory and capital markets.',
  });
}
...
const defaultWhyChoose: Array<[string, string, React.ElementType]> = [
...
const defaultTeam = [
  {
    ...
    shortBio:
      'Over 30 years of experience in cross-border capital markets and global regulatory strategy, advising Fortune 500 boards on institutional risk.',
  ...
  },
  {
    ...
    shortBio:
      'Formerly a senior advisor at the European Central Bank, specializing in cross-border compliance and systemic risk frameworks.',
  ...
  },
  {
    ...
    shortBio:
      'Directing global portfolio strategy and macro-economic forecasting for institutional clients across four continents.',
  ...
  },
  {
    ...
    shortBio:
      'Expert in multi-stakeholder engagement and international partnership structuring across emerging and developed markets.',
...
export default async function AboutPage() {
  const [hero, dbTeam, dbOffices, cms] = await Promise.all([
  ...
  const valuesSec = cms.sectionMap['values'];
  const whyChooseSec = cms.sectionMap['why-choose'];
  const awardsSec = cms.sectionMap['awards'];
  const ctaSec = cms.sectionMap['cta'];

  const valuesItems = (valuesSec?.content as Array<[string, string]>) || defaultValues;
  const awardsItems = (awardsSec?.content as any)?.awards || defaultAwards;
  const certificationsItems = (awardsSec?.content as any)?.certifications || defaultCertifications;
    ...
    <main id="top">
      ...
      <AnimatedSection className="mx-auto flex max-w-7xl flex-col justify-center px-6 pb-16 pt-10 lg:px-10">
        ...
        <p className="mt-6 max-w-2xl text-base leading-7 text-slate-600 lg:text-lg">
          {hero.subheading ||
            'For over two decades, PSC Global has served as the trusted advisory partner to institutions navigating the most consequential decisions of their global expansion.'}
        </p>
        <img
      ...
      <AnimatedSection className="mx-auto grid max-w-7xl items-center gap-12 border-t border-slate-200 px-6 py-20 md:grid-cols-2 lg:px-10">
        <div>
          <p className="font-serif text-4xl lg:text-5xl">{whoWeAreSec?.title || 'Who We Are'}</p>
          <p className="mt-6 font-serif text-xl italic leading-8 text-slate-700 lg:text-2xl">
            &ldquo;{whoWeAreSec?.subtitle || 'Our commitment is not merely to offer counsel, but to provide a foundational architecture for our clients’ global ambitions.'}&rdquo;
          </p>
          <p className="mt-6 text-base leading-8 text-slate-600 lg:text-lg">
            {whoWeAreSec?.bodyContent ||
              "Founded on the principles of absolute precision and unyielding integrity, PSC Global has emerged as a preeminent advisory firm for the world's most sophisticated institutions. We operate at the intersection of regulatory complexity and strategic opportunity, providing the clarity necessary for long-term success in an ever-shifting global landscape."}
          </p>
        </div>
      ...
      <AnimatedSection className="mx-auto flex max-w-7xl flex-col justify-center border-t border-slate-200 px-6 py-20 lg:px-10">
        <p className="font-mono text-xs tracking-[.18em] text-slate-500">{valuesSec?.eyebrow || 'OUR CORE'}</p>
        <div className="mt-4 flex items-center justify-between">
          <h2 className="font-serif text-5xl lg:text-6xl">{valuesSec?.title || 'Vision & Values'}</h2>
          <span className="hidden h-px w-32 bg-slate-300 md:block" />
        </div>
        <div className="mt-14 flex flex-wrap justify-center gap-y-14">
          {valuesItems.map(([title, copy], i) => (
            <div
              key={title}
      ...
      <AnimatedSection className="flex items-center border-t border-slate-200 bg-slate-50 px-6 py-20">
        <div className="mx-auto max-w-7xl">
          <h2 className="text-center font-serif text-5xl lg:text-6xl">{whyChooseSec?.title || 'Why Businesses Choose PSC'}</h2>
          <p className="mt-4 text-center text-sm text-slate-500 lg:text-base">
            {whyChooseSec?.subtitle || 'Providing a distinctive edge through strategic alignment and technical mastery.'}
          </p>
          <div className="mt-10 grid gap-3 md:grid-cols-3">
            {defaultWhyChoose.map(([title, copy, Icon]) => (
              <article
      ...
      {/* AWARDS & RECOGNITIONS */}
      <AnimatedSection className="mx-auto flex max-w-7xl flex-col justify-center border-t border-slate-200 px-6 py-20 lg:px-10">
        <h2 className="text-center font-serif text-5xl lg:text-6xl">{awardsSec?.title || 'Awards & Recognitions'}</h2>
        <div className="mt-10 grid gap-3 sm:grid-cols-4">
          {awardsItems.map(([big, label]: [string, string]) => (
        ...
        <div className="mt-16 overflow-hidden border-y border-slate-200 py-10">
          <div className="flex w-max animate-marquee gap-16">
            {[...certificationsItems, ...certificationsItems].map((x: string, i: number) => (
        ...
        <div className="mx-auto grid max-w-7xl gap-12 px-6 py-20 md:grid-cols-2 lg:px-10">
          <div>
            ...
            <p className="mt-6 max-w-sm text-base leading-7 text-slate-300 lg:text-lg">
              We&apos;re strategically positioned across the world&apos;s most vital financial hubs, ensuring that our expertise is
              always within reach.
            </p>
            {offices.map((o) => (
              <div className="mt-4 flex flex-col border-b border-slate-700/80 pb-3" key={o.id}>
                <div className="flex items-center justify-between text-sm tracking-widest font-bold">
                  <span>
                    {o.city.toUpperCase()} {o.isHeadquarters ? '(HQ)' : ''}
                  </span>
                  <ArrowRight size={16} />
                </div>
          ...
          <OfficeMap offices={offices} coordinates={officeCoordinates} label="PSC GLOBAL | GLOBAL REACH & CONNECTIVITY" />
        </div>
      </AnimatedSection>
      ...
      <AnimatedSection className="flex flex-col items-center justify-center border-t border-slate-200 px-6 py-24 text-center">
        <h2 className="font-serif text-5xl leading-[1.05] lg:text-6xl">
          {ctaSec?.title || "Let's Build Your Next Chapter Together."}
        </h2>
        <p className="mx-auto mt-4 max-w-lg text-sm text-slate-600 lg:text-base">
          {ctaSec?.subtitle || "Discover how PSC Global's integrated advisory can transform your strategic position."}
        </p>
