...
const defaultTeam = [
  {
    ...
    slug: 'adrian-thorne',
    name: 'Adrian Thorne',
    roleTitle: 'Partner, Infrastructure & Energy',
    shortBio: 'Expert in long-term project finance and regulatory structuring for emerging energy markets.',
  ...
  },
  {
    ...
    slug: 'elena-rostova',
    name: 'Elena Rostova',
    roleTitle: 'Director, Tech & Startups',
    shortBio: 'Advising on scale-up strategies, cross-border IP protection, and venture capital compliance.',
  ...
  },
  {
    ...
    slug: 'marcus-vane',
    name: 'Marcus Vane',
    roleTitle: 'Head of Banking & Finance',
    shortBio: 'Strategic advisor for global financial institutions navigating digital transformation and risk.',
...
const insights = [
  {
    ...
    title: 'The Impact of New Global Carbon Tax Frameworks on Manufacturing.',
    image: 'https://images.unsplash.com/photo-1487958449943-2429e8be8625?auto=format&fit=crop&w=700&q=80',
  },
  {
    tag: 'INDUSTRY INSIGHT',
    title: 'Digital Sovereignty: How Tech Firms Are Navigating EU Data Laws.',
    image: 'https://images.unsplash.com/photo-1512314889357-e157c22f938d?auto=format&fit=crop&w=700&q=80',
  },
  {
    tag: 'CASE STUDY',
    title: 'Scaling Real Estate Portfolios in High Inflation Environments.',
    image: 'https://images.unsplash.com/photo-1449844908441-8829872d2607?auto=format&fit=crop&w=700&q=80',
  },
] as const;
...
export default async function IndustriesPage() {
  ...
  const industries = dbIndustries.length > 0 ? dbIndustries : defaultIndustries;
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-14 grid gap-10 md:grid-cols-3">
            {team.slice(0, 3).map((m: any) => (
              <Link href={`/partner/${m.slug}`} key={m.id} className="profile-card block">
                <img
                ...
                <p className="mt-2 text-sm leading-6 text-slate-600">{m.shortBio}</p>
              </Link>
            ))}
          </div>
        </div>
          ...
          <div className="mt-12 grid gap-8 md:grid-cols-3">
            {insights.map((post) => (
              <div key={post.title}>
                ...
                <h3 className="mt-2 font-serif text-xl leading-snug text-ink">{post.title}</h3>
              </div>
            ))}
          </div>
        </div>
