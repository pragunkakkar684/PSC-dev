    ...
    <section id="sectors" className="bg-navy text-white">
      <div className="mx-auto max-w-7xl px-6 py-18 lg:px-10">
        ...
        <div className="mt-9 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {hasDbData
            ? data.map((ind) => {
                const img = ind.imageUrl || 'https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?auto=format&fit=crop&w=700&q=70';
                return (
                  <article
                    className="flex h-52 items-end bg-cover bg-center p-5 transition hover:-translate-y-1 hover:brightness-110"
                    style={{ backgroundImage: `linear-gradient(0deg,rgba(0,18,38,.95),rgba(0,18,38,.08)),url(${img})` }}
                    key={ind.id}
                  >
                    <span className="font-serif text-xl">{ind.name}</span>
                  </article>
                );
              })
